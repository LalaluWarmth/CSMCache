 ...
 * 要实现CSM效果, 我们需要每帧根据相机的近切远切面更新平行光远光锥, 并将不同距离的相机视锥体中的可见物体绘制到不同的阴影(深度)图，然后在实际进行阴影绘制时，根据物体到相机的距离采样对应的深度图并检测阴影
 * 而要实现CSMCache效果，我们则需要将静态物体与动态物体区分，然后将那些不会移动，因而也不需要重新绘制阴影图的对象们缓存起来，然后每帧仅检测移动的物体, 将其与静态的阴影图加总即可
*/
...
#pragma region CSM
    //添加一个用于阴影贴图的深度/模板缓冲区. 为了CSM, 其最多要是4个了
    // Add +4 DSV for shadow map.
...
#pragma region CSM
	texTable0.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 5, 0, 0);   //由于CSM, 现在阴影图的Srv最多可能是4个了

	CD3DX12_DESCRIPTOR_RANGE texTable1;
	texTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 10, 5, 0);  //因此, texTable1从绑定t2换为绑定到t5
#pragma endregion
